...
const CrossMatchCancelConfirm = forwardRef(({ apiData, isConfirmation }, ref) => {
  ...
  useEffect(() => {
    if (isConfirmation) {
      ...
      console.log("Formatted API Payload:", JSON.stringify(selectedData, null, 2));
    }
  ...
  Object.entries(selectedBags).forEach(([requisitionNo, bagSet]) => {
    const bags = Array.from(bagSet).map((bloodBagNo) => {
      ...
      const verificationStatusValue = verificationStatus[statusKey] || "-1";
    ...
    if (bags.length > 0) {
        ...
        const compoString = bags.map(bag => bag.componentid).join('#') + '#';
 ...
 const result = Object.values(requisitionMap);
    ...
    // Add console log only for confirmation route
    if (isConfirmation) {
      console.log("Selected Bags Data for Verification API:", JSON.stringify(result, null, 2));
    }
              ...
              >
                ...
                <Select
                  ...
                  options={[
                    ...
                    { value: "3", label: "Revoke" },
                  ]}
                  value={requisitionStatus[requisitionno] || "Pending"}
                   onChange={(value) => handleRequisitionStatusChange(requisitionno, value)}
                  onClick={(e) => e.stopPropagation()}
                />
              </div>
            </div>
          ...
          <Table
            columns={[
                  ...
                  return (
                    <div onClick={(e) => e.stopPropagation()}>
                      ...
                      <Select
                        defaultValue="-1"
                        value={
                          verificationStatus[statusKey] || "Pending Status"
                        }
                        onChange={(value) =>
                          handleStatusChange(
                            record.requisitionno,
              ...
              },
              {
                ...
                render: (text, record) => (
                  <Input size="small" placeholder="Cancellation Remark" disabled />
                ),
              },
              {
            ...
            <>
              <div className="table_header table_row p-2 mb-2">
                <div className="row w-100 align-items-center">
                  ...
                  <div className="col-2">
                    <Input placeholder="Verification Remark for all" />
                  </div>
                </div>
              </div>
